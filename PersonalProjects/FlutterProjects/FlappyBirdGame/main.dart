...
class _FlappyBirdState extends State<FlappyBird> with SingleTickerProviderStateMixin {
  ...
  void startGame() {
    ...
    pipeTimer = Timer.periodic(const Duration(milliseconds: 1300), (_) => placePipes());
  ...
  void placePipes() {
    ...
    double randomPipeY = -pipeHeight / 4 - Random().nextDouble() * pipeHeight / 2;

    pipes.add(Pipe(x: boardWidth, y: randomPipeY, isTop: true));
    pipes.add(Pipe(x: boardWidth, y: randomPipeY + pipeHeight + openingSpace, isTop: false));
  }
